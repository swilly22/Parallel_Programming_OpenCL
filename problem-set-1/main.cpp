...
int main(int argc, const char * argv[]) {
    ...
    // Alocate buffer which will receive gray scale values.
    unsigned char* intensityPixels = (unsigned char*) malloc(sizeof(unsigned char) * image.cols * image.rows * 4);
    ...
    // Image which will receive gray scale values.
    ...
    dispatch_sync(queue, ^{
        ...
        // Calling the kernel.
        grayScale_kernel(&range, dInputImage, dOutputImage);

        // Read back the results.
        gcl_copy_image_to_ptr(intensityPixels, dOutputImage, origin, region);
        
    });
    
    // Set gray scale values to image.
    pixelIdx = 0;
    // Foreach row
    for(int i = 0; i < imageHeight; i++){
        pixel = image.ptr<uchar>(i);
        // Foreach column
        for(int j = 0; j < imageWidth; j+=3){
            // All 3 channels receive the same intensity value.
            pixel[j] = intensityPixels[pixelIdx];
            pixel[j+1] = intensityPixels[pixelIdx+1];
            pixel[j+2] = intensityPixels[pixelIdx+2];
            pixelIdx += 4; // Move to the next pixel.
        }
    }
